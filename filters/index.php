...
if (\Bitrix\Main\Loader::includeModule('iblock')) {
	...
	// сбор инфо обо всех свойствах товара
	$propInfoList = [];
	while ($arData = $rsData->GetNext()) {
		$propInfoList[$arData['CODE']] = $arData;
	}

	if ($requestArr && $propInfoList) {

		foreach ($requestArr as $param => $value) {

			if (!$value) continue;

			// если параметр это свойство элемента
			if (key_exists($param, $propInfoList)) {
				...
				// если тип фильтра RANGE
				if (!is_array($value)) continue;

				$param = str_replace('_R', '', $param);
				$propCode = $propInfoList[$param]['CODE'];
				$arrFilter['>=PROPERTY_' . $propCode] = $value[0] ?: 0;
				$arrFilter['<=PROPERTY_' . $propCode] = $value[1] ?: pow(10, 6);
			}
		}
	}
}
